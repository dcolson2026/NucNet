...
)
{
  ...
  for( i = 2; i < WnMatrix__get_gsl_vector_size( p_z_vector ); i++ )
  {
    ss_xpath.str( "" );
    ss_xpath.clear();
    ss_xpath << boost::format( "[z = %lu]" ) % (unsigned long) i;
    p_cluster = Libnuceq__getCluster( p_equil, ss_xpath.str().c_str() );
    if( !p_cluster )
      p_cluster = Libnuceq__newCluster( p_equil, ss_xpath.str().c_str() );
  ...
  ss_xpath.str( "" );
  ss_xpath.clear();
  ss_xpath << boost::format( "[z = %d]" ) % (unsigned long) i_Z;
  
  std::cout <<
    boost::format( "For step number %s, T9 = %s, rho (g/cc) = %s\n" ) %
  ...
  std::cout <<
    boost::format( "Z = %d, mu/kT = %g\n" ) %
      i_Z %
      Libnuceq__Cluster__getMukT(
        Libnuceq__getCluster( p_equil, ss_xpath.str().c_str() )
      );
