...
check_input( int argc, char **argv )
{
  ...
  if( argc < 4 || argc > 6 )
  {
    fprintf(
      stderr,
      "\nUsage: %s nuc_file zone_ascii zone_out abund_xml abund_xpath\n\n",
      argv[0]
    );
    fprintf(
    ...
    fprintf(
      stderr, "  abund_xml = input abundance xml filename (optional)\n\n"
    );
    fprintf(
    ...
    fprintf(
      stderr,
      "  abund_xml = input abundance xml filename (optional--required if abund_xml present)\n\n"
    );
    fprintf(
      stderr, "  abund_xpath = input abundance xml filename (optional)\n\n"
    );
...
int main( int argc, char * argv[] ) {
  ...
  if( argc >= 5 )
  {
    if( argc == 5 )
    {
      Libnucnet__assignZoneDataFromXml( p_my_nucnet, argv[4], NULL );
    }
    else
    {
      Libnucnet__assignZoneDataFromXml( p_my_nucnet, argv[4], argv[5] );
    }
    nnt::zone_list_t zone_list = nnt::make_zone_list( p_my_nucnet );
    if( zone_list.size() != 1 )
    {
      std::cerr << "Invalid zone input from xml.\n";
      exit( EXIT_FAILURE);
    }
    p_abunds =
  ...
  //============================================================================
  // Remove abundance zone data, if present.
  //============================================================================
