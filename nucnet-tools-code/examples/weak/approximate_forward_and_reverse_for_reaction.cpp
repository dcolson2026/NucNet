...
//!   reaction at a given input based on Arcones et al., A&A 522, A25
//!   (2010).
////////////////////////////////////////////////////////////////////////////////
...
int main( int argc, char * argv[] ) {
  ...
  std::string s_purpose =
    "\nPurpose: compute weak rates for reaction from Arcones et al. (2010) using input nuc_xml file.";
  ...
  try
  {
    ...
    if( argc < 3 || vm.count("help") == 1 )
    {
      std::cerr <<
        "\nUsage: " <<
           argv[0] << " nuc_xml reaction [options]" << std::endl;
  ...
  if( !p_reaction )
  {
    std::cerr << "Reaction " << argv[2] << " not found." << std::endl;
    return EXIT_FAILURE;
  }
