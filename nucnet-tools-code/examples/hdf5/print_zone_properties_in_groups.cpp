...
////////////////////////////////////////////////////////////////////////////////
//! \file
//! \brief Example code to print out properties for a given zone in all groups.
////////////////////////////////////////////////////////////////////////////////
...
check_input( int argc, char **argv )
{
  ...
  if( argc < 6 )
  {
    ...
    std::cout << "Purpose: " << argv[0] <<
      " prints out selected properties for a zone in all group ";
    std::cout <<
...
main( int argc, char **argv )
{
  ...
  std::vector<std::string> groups =
    user::hdf5::get_group_names_in_file( argv[1] );

  BOOST_FOREACH( std::string group_name, groups )
  {

    my_zone_labels =
      user::hdf5::get_zone_labels( argv[1], group_name.c_str() );
    ...
    BOOST_FOREACH( std::vector<std::string> property, property_list )
    {
      ...
      if( it2 == my_properties_hash.end() )
      {
        std::cerr << "\nProperty " << property[0] << ", " <<
                     property[1] << ", " <<
                     property[2] << " not present.\n";
        return EXIT_FAILURE;
      }
