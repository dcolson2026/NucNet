...
class entropy_generation_rhs
{
  ...
  public:
    ...
    )
    {
      ...
      double d_dt, d_entropy_generation;
...
get_input( int argc, char **argv )
{
  ...
  try
  {
    ...
    if( vm.count("sdot_nuc_xpath") == 1 )
    {
      s_sdot_nuc_xpath = vm["sdot_nuc_xpath"].as<std::string>();
    }

    if( vm.count("sdot_reac_xpath") == 1 )
    {
      s_sdot_reac_xpath = vm["sdot_reac_xpath"].as<std::string>();
    }
    ...
    if( vm.count("sdot_nuc_xpath") == 1 || vm.count("sdot_reac_xpath") == 1 )
    {
      p_view =
