...
typedef
boost::function<result_t( nnt::Zone&, Libnucnet__NetView *)> eg_function_t; 
...
boost::tuple<Libnucnet *, std::string, std::string, eg_function_t, double>
  ...
  try
  {
    ...
    desc.add_options()
      ...
      (
       "cutoff",
       po::value<double>(),
       "cutoff for rate printout (default: 1.e-25)"
      )
    ;
    ...
    if( s_eg_type == "nucleon" )
    {
      my_function =
        static_cast<eg_function_t>(
          boost::bind(
            user::compute_zone_reactions_energy_generation_rate_per_nucleon,
            _1,
            _2
          )
        );
    ...
    else if( s_eg_type == "gram" )
    {
      my_function =
        static_cast<eg_function_t>(
          boost::bind(
            user::compute_zone_reactions_energy_generation_rate_per_gram, _1, _2
          )
        );
    }
    else
    ...
    return
      boost::make_tuple(
        p_nucnet, s_nuc_xpath, s_reac_xpath, my_function, d_cutoff
      );
...
int main( int argc, char * argv[] ) {
  ...
  for( size_t i = 0; i < zones.size(); i++ )
  {
    ...
    std::sort( results.begin(), results.end(), my_sorter );

    std::cout <<
      boost::format( "time(s) = %g  t9 = %g rho(g/cc) = %g\n\n" ) %
    ...
    {
      if( fabs( results[j].second ) >= my_tuple.get<4>() )
      {
        std::cout << 
          boost::format( "%50s  %e\n" ) %
            results[j].first %
