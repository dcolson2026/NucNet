...
)
{
  ...
  std::map<std::string,my_graph_t::vertex_descriptor> my_vertices;
  std::map<std::string,std::string> special_vertex_color_map;
  std::map<std::string,std::string>::iterator its;
  std::string s_shape = "box";

  std::pair<std::string,std::string> colors = solar_color();

  BGL_FORALL_VERTICES( vi, g, my_graph_t )
  {
...
)
{
  ...
  BGL_FORALL_EDGES( ei, g, my_graph_t )
  {
...
)
{
  ...
  BGL_FORALL_EDGES( e, g, my_graph_t )
  {
    if( fabs( g[e].getWeight() ) > d_max ) d_max =
      fabs( g[e].getWeight() );
  }
  ...
  if( d_net_flow >= 0 )
  {
	...
	my_vertex_hash_t::iterator itp =
	  vertices_hash.find(
        ...
        boost::tie( e, b_edge ) = boost::edge( itr->v, itp->v, g );
	...
	if( !b_edge )
	{
	  ...
	  g[e].setWeight( d_net_flow );
        ...
        {
          g[e].setWeight( g[e].getWeight() + d_net_flow );
        }
  ...
  else
  {
	...
	my_vertex_hash_t::iterator itr =
	  vertices_hash.find(
        ...
        boost::tie( e, b_edge ) = boost::edge( itp->v, itr->v, g );
...
)
{
  ...
  BOOST_FOREACH( nnt::ReactionElement reactant, reactant_list )
  {
	...
	g[e].setWeight( flows.first );
      ...
      {
        g[e].setWeight( g[e].getWeight() + flows.first );
      }
  ...
  BOOST_FOREACH( nnt::ReactionElement product, product_list )
  {
	...
	g[e].setWeight( flows.second );
      ...
      {
	g[e].setWeight( g[e].getWeight() + flows.second );
      }
