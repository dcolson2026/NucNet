...
)
{
  ...
  user::flow_data_tuple_t data_tuple = user::make_flow_data_tuple( zone );
...
main( int argc, char **argv )
{
  ...
  for( size_t i = 0; i < zones.size(); i++ )
  {
    ...
    std::cout <<
      Libnucnet__Zone__getLabel( zones[i].getNucnetZone(), 1 ) << std::endl;

    user::update_rate_functions_data( zones[i] );

    my_graph_t g = create_graph( zones[i], p_view, argv[6] );
    ...
    my_output( sub_g, zones[i], s_output.c_str() );
