...
)
{
  ...
  if( zones[0].hasProperty( nnt::s_ITER_SOLVER ) )
  {
    WnSparseSolve__Mat__updateSolverMethod(
      p_solver,
      zones[0].getProperty<std::string>( nnt::s_ITER_SOLVER ).c_str()
    );
  }
  else
  {
  ...
  if( zones[0].hasProperty( nnt::s_ITER_SOLVER_MAX_ITERATIONS ) )
  {
    WnSparseSolve__Mat__updateMaximumIterations(
      p_solver,
      zones[0].getProperty<int>( nnt::s_ITER_SOLVER_MAX_ITERATIONS )
    );
  }
  ...
  if( zones[0].hasProperty( nnt::s_ITER_SOLVER_REL_TOL ) )
  {
    WnSparseSolve__Mat__updateRelativeTolerance(
      p_solver,
      zones[0].getProperty<double>( nnt::s_ITER_SOLVER_REL_TOL )
    );
  }
  else
  {
  ...
  if( zones[0].hasProperty( nnt::s_ITER_SOLVER_ABS_TOL ) )
  {
    WnSparseSolve__Mat__updateAbsoluteTolerance(
      p_solver,
      zones[0].getProperty<double>( nnt::s_ITER_SOLVER_ABS_TOL )
    );
  }
  else
  {
  ...
  if( zones[0].hasProperty( nnt::s_ITER_SOLVER_MAX_ITERATIONS ) )
  {
    WnSparseSolve__Mat__updateMaximumIterations(
      p_solver,
      zones[0].getProperty<double>( nnt::s_ITER_SOLVER_MAX_ITERATIONS )
    );
  }
  else
  {
  ...
  if( zones[0].hasProperty( nnt::s_ITER_SOLVER_CONVERGENCE_METHOD ) )
  {
    WnSparseSolve__Mat__updateConvergenceMethod(
      p_solver,
      zones[0].getProperty<std::string>(
  ...
  if(
    zones[0].hasProperty( nnt::s_ITER_SOLVER_DEBUG ) &&
    zones[0].getProperty<std::string>( nnt::s_ITER_SOLVER_DEBUG ) == "yes"
  )
  {
  ...
  {
    i_delta = zones[0].getProperty<size_t>( nnt::s_ILU_DELTA );
  }
  else
  {
    i_delta = 50;
  ...
  if( zones[0].hasProperty( nnt::s_ILU_DROP_TOL ) )
  {
    d_droptol = zones[0].getProperty<size_t>( nnt::s_ILU_DROP_TOL );
  }
  else
  {
    d_droptol = 0;
  ...
  return std::make_pair<size_t, double>( i_delta, d_droptol );
...
evolve_zones( std::vector<nnt::Zone>& zones, double dt )
{
  ...
  user::safe_evolve_multi_zone(
    ...
    boost::bind( set_solver_properties, _1, boost::ref( zones ) ),
    boost::bind(
