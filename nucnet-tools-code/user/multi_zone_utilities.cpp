...
create_full_vector( std::vector<nnt::Zone>& zones )
{
  ...
  is_multi_mass = is_multi_mass_calculation( zones );
  ...
  for( size_t i = 0; i < zones.size(); i++ )
  {
    ...
    if( is_multi_mass )
    {
      ...
      p_work = Libnucnet__Zone__getMassFractions( zones[i].getNucnetZone() );
    ...
    {
      p_work = Libnucnet__Zone__getAbundances( zones[i].getNucnetZone() );
    }
    ...
    gsl_vector_memcpy( &view.vector, p_work );
...
)
{
  ...
  std::set<Libnucnet__Zone *> mm_set, n_mm_set;

  BOOST_FOREACH( nnt::Zone zone, zones )
  {
    if( zone.hasProperty( nnt::s_ZONE_MASS ) )
    {
      mm_set.insert( zone.getNucnetZone() );
    }
    else
    {
      n_mm_set.insert( zone.getNucnetZone() );
    }
  }

  if( mm_set.size() != 0 && n_mm_set.size() == 0 )
  {
    return true;
  }
  else if( mm_set.size() == 0 && n_mm_set.size() != 0 )
  {
    return false;
  }
  else
  {
    std::cerr << "Expecting multi-mass calculation but the following\n" <<
                 "zones do not have a zone mass:\n\n";
    BOOST_FOREACH( Libnucnet__Zone * p_zone, n_mm_set )
    {
      std::cerr << 
        "(" <<
