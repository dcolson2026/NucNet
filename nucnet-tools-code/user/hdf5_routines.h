...
typedef struct nuclide
{
  ...
  unsigned int iZ;
  unsigned int iA;
  char sSource[I_HDF5_BUF];
  int iStateFlag;
  char sState[I_HDF5_BUF];
  double dSpin;
  double dMassExcess;
} nuclide;
...
get_group_mass_fractions( const char *, const char * );
...
std::string create_group_label( size_t );

std::vector<std::string>
get_group_names_in_file( const char * );
