...
)
{
  ...
  std::string s_group = create_group_label( count_groups_in_file( s_file ) );
...
write_nuclide_data( H5::H5File& file, Libnucnet * p_nucnet )
{
  ...
  BOOST_FOREACH( nnt::Species species, species_list )
  {
    ...
    strcpy(
      my_nuclide.sSource,
      Libnucnet__Species__getSource( species.getNucnetSpecies() )
    );
    if( Libnucnet__Species__getStateString( species.getNucnetSpecies() ) )
    {
      strcpy(
        my_nuclide.sState,
        Libnucnet__Species__getStateString( species.getNucnetSpecies() )
      );
    }
    else
    {
      strcpy( my_nuclide.sState, "" );
    }
    my_nuclide.dMassExcess =
      Libnucnet__Species__getMassExcess( species.getNucnetSpecies() );
    my_nuclide.dSpin =
      Libnucnet__Species__getSpin( species.getNucnetSpecies() );
  ...
  my_type.insertMember( S_STATE, HOFFSET( nuclide, sState ), string_type );
  my_type.insertMember( S_SOURCE, HOFFSET( nuclide, sSource ), string_type );
  my_type.insertMember(
    S_MASS_EXCESS,
...
)
{
  ...
  group = my_file.openGroup( s_group );
...
)
{
  ...
  my_zone_labels = get_zone_labels( s_file, s_group );
...
)
{
  ...
  std::vector<std::string> * p_names = ( std::vector<std::string> * ) p_data;
...
)
{
  ...
  H5::Group group = file.openGroup( s_group );

  try
  {
    file.unlink( s_group );
  }
  catch( H5::FileIException const& unlink_error )
  {
    std::cerr << "Can't remove group " << s_group << std::endl;
  }
