...
////////////////////////////////////////////////////////////////////////////////
// HOW TO RECOMPILE SAFELY: make clean, cleanall_network, all_network
#include "hydro.h"
...
void // the one that matters
update_zone_properties(nnt::Zone &zone)
{
  std::cout << "the one we WANT" << std::endl;

  double d_t, d_tau_0 = 0;
  d_t = zone.getProperty<double>(nnt::s_TIME);
  // std::cout << boost::lexical_cast<double>(zone.getProperty<double>("tau_1")) << std::endl;

  // everything surrounded here is new
  /////////////////////////////////////////////
  try
  {
    d_tau_0 = boost::lexical_cast<double>(zone.getProperty<double>("tau_0"));
  }
  catch (const boost::bad_lexical_cast &e)
  {
    if (zone.getProperty<std::string>("tau_0") == "inf")
    {
      return;
    }
    else
  ...
  // removed d_tau_1 from initialized variables at top then uncommented this
  // try
  // {
  //   d_tau_1 = boost::lexical_cast<double>(zone.getProperty<double>("tau_1"));
  // }
  // catch (const boost::bad_lexical_cast &e)
  // {
  //   if (zone.getProperty<std::string>("tau_1") == "inf")
  //   {
  //     return;
  //   }
  //   else
  ...
  double d_rho =
      boost::lexical_cast<double>(zone.getProperty<double>(nnt::s_RHO_0)) * exp(-d_t / d_tau_0) + boost::lexical_cast<double>(zone.getProperty<double>("tau_1")) + boost::lexical_cast<double>(zone.getProperty<double>("rho_1")) / pow(1. + d_t / boost::lexical_cast<double>(zone.getProperty<double>("tau_1")), 2.);
  zone.updateProperty(nnt::s_RHO, boost::lexical_cast<std::string>(d_rho));

  double d_t9 =
      boost::lexical_cast<double>(zone.getProperty<double>(nnt::s_T9_0)) *
      pow(d_rho / (boost::lexical_cast<double>(zone.getProperty<double>(nnt::s_RHO_0)) + boost::lexical_cast<double>(zone.getProperty<double>("rho_1"))), 1. / 3.);
  zone.updateProperty(nnt::s_T9, boost::lexical_cast<std::string>(d_t9));
  /////////////////////////////////////////////
  ...
  // trying r-process
  // uncomment above stuff to restore
}
...
// update_zone_properties( nnt::Zone& zone )
// {
//   std::cout << "hopefully not here?" << std::endl;
...
//   if( zone.getProperty<double>( nnt::s_RHO ) < 1.e-69 )
//     zone.updateProperty(
//       nnt::s_RHO,
//       "1.e-30"
//     );
