...
DATA=np.loadtxt("frdm_2012.txt")
N_data = DATA[:, 1]
A_data = DATA[:, 2]
mass_excess_data = DATA[:, 14]
print(len(N_data), len(A_data), len(mass_excess_data))
num_entries = len(N_data)

# need data as "Z   A   mass_excess(MeV)"
fw = open('new_mass_excesses.txt','w')
for i in range(num_entries):
    N = int(N_data[i])
    A = int(A_data[i])
    mass_excess = float(mass_excess_data[i])
    Z = A - N
    if Z > 112: # no nuclide data past 112
        continue

    print(f"{Z}   {A}   {mass_excess}", file = fw)
