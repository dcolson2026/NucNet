...
namespace my_user
{
  ...
  get_nucnet(int argc, char **argv)
  {
    ...
    {
      std::cout << std::endl;
      std::cout << argv[0] << " runs a single-zone network calculation for matter expanding" << " exponentially." << std::endl
                << std::endl;
      std::cout << "For a usage statement, type " << std::endl
                << std::endl;
      std::cout << argv[0] << " --usage" << std::endl
                << std::endl;
      std::cout << "For an example usage, type " << std::endl
                << std::endl;
      std::cout << argv[0] << " --example" << std::endl
                << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    {
      std::cout << std::endl;
      std::cout << argv[0] << " ../../data_pub/my_net.xml " << "../../data_pub/zone.xml my_output.xml \"[z <= 20]\"" << std::endl
                << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  update_zone_properties(nnt::Zone &zone)
  {
    ...
    // double d_t9 = boost::lexical_cast<double>(zone.getProperty(nnt::s_T9_0)) *
    //               boost::lexical_cast<double>(zone.getProperty(nnt::s_RHO_0)) /
    //               (boost::lexical_cast<double>(zone.getProperty(nnt::s_RHO_0)) + d_tau_0 * d_t);
    ...
    //         1. / 3.);
    // CHANGED location of this block so d_rho was defined

    // zone.updateProperty(
    //     nnt::s_T9,
    //     boost::lexical_cast<std::string>(d_t9));
    // CHANGED location so it would be defined
    ...
    // double fT9=d_t9*d_t9/(d_t9*d_t9+5.3);// K.Farouqi AJ2010
    // double d_rho =100000*1.21*pow(d_t9,3)*(1.+7./4.*fT9)/d_tau_1;

    //============================================================================
    // Handle extremely low temperature and density.
    //============================================================================

    // CHANGED this to block it all off.
    //   if(
